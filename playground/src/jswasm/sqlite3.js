...
let sqlite3InitModule = (() => {
  ...
  return (
    function (config) {
      let sqlite3InitModule = config || {}

      'use strict'

      let Module = typeof sqlite3InitModule != 'undefined' ? sqlite3InitModule : {}
      ...
      var FS = {
        ...
        saveFilesToDB: (paths, onload, onerror) => {
          ...
          openRequest.onsuccess = () => {
            ...
            let ok = 0; let fail = 0; let total = paths.length
            function finish() {
              if (fail == 0) {
                onload()
              } else {
      ...
      let MAX_INT53 = 9007199254740992

      let MIN_INT53 = -9007199254740992
      ...
      function ___syscall_ftruncate64(fd, length) {
        try {
          length = bigintToI53Checked(length); if (isNaN(length)) {
            return -61
          }
      ...
      let __MONTH_DAYS_LEAP_CUMULATIVE = [0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335]

      let __MONTH_DAYS_REGULAR_CUMULATIVE = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334]
      function __yday_from_date(date) {
        let isLeapYear = __isLeapYear(date.getFullYear())
        let monthDaysCumulative = (isLeapYear ? __MONTH_DAYS_LEAP_CUMULATIVE : __MONTH_DAYS_REGULAR_CUMULATIVE)
        let yday = monthDaysCumulative[date.getMonth()] + date.getDate() - 1

        return yday
      }
      function __localtime_js(time, tmPtr) {
        let date = new Date(readI53FromI64(time) * 1000)
        ...
        let yday = __yday_from_date(date) | 0
        HEAP32[(((tmPtr) + (28)) >> 2)] = yday
        HEAP32[(((tmPtr) + (36)) >> 2)] = -(date.getTimezoneOffset() * 60)

        let start = new Date(date.getFullYear(), 0, 1)
        let summerOffset = new Date(date.getFullYear(), 6, 1).getTimezoneOffset()
        let winterOffset = start.getTimezoneOffset()
        let dst = (summerOffset != winterOffset && date.getTimezoneOffset() == Math.min(winterOffset, summerOffset)) | 0
      ...
      function __tzset_js(timezone, daylight, tzname) {
        ...
        function extractZone(date) {
          let match = date.toTimeString().match(/\(([A-Za-z ]+)\)$/)
          return match ? match[1] : 'GMT'
        }
        let winterName = extractZone(winter)
        let summerName = extractZone(summer)
        let winterNamePtr = allocateUTF8(winterName)
        let summerNamePtr = allocateUTF8(summerName)
      ...
      function getEnvStrings() {
        if (!getEnvStrings.strings) {
          let lang = `${((typeof navigator == 'object' && navigator.languages && navigator.languages[0]) || 'C').replace('-', '_')}.UTF-8`
          let env = {
            USER: 'web_user',
            LOGNAME: 'web_user',
            PATH: '/',
      ...
      function _fd_seek(fd, offset, whence, newOffset) {
        try {
          offset = bigintToI53Checked(offset); if (isNaN(offset)) {
            return 61
          }
          let stream = SYSCALLS.getStreamFromFD(fd)
  ...
  apiConfig = (self.sqlite3ApiConfig || sqlite3ApiBootstrap.defaultConfig),
    ...
    = (t, v) => (t === null ? v : (t ? xResult.get(t)(v) : undefined))
        ...
        || toss('Function pointer not found in WASM function table.')
            ...
            if (undefined !== resultType && resultType !== null) {
      ...
      = capi.sqlite3_create_window_function.udfSetError = capi.sqlite3_result_error_js
        ...
        = wasm.installFunction('i(ippp)', function (t, c, p, x) {
          ...
          const dbCtorHelper = function ctor(...args) {
            ...
            let fn = opt.filename; let vfsName = opt.vfs; let flagsStr = opt.flags
        ...
        = (db, sql, bind, rowMode) => db.exec({
         ...
         && ((typeof pApp !== 'number') || !util.isInt32(pApp))) {
          ...
          const isSupportedBindType = function (v) {
            let t = BindTypes[(v === null || undefined === v) ? 'null' : typeof v]
            switch (t) {
              ...
              if (opt.catchExceptions) {
                ...
                for (const k of mnames) {
                  ...
                  if (!(m instanceof Function)) {
                    continue
                  } else if (k === 'xConnect' && methods.xCreate === m) {
