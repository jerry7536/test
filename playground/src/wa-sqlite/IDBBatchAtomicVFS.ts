...
export class IDBBatchAtomicVFS extends VFS.Base {
  ...
  /** @type {IDBContext} */ #idb: IDBContext
  /** @type {Set<string>} */ #pendingPurges: Set<string> = new Set()
  private name: string
  ...
  xOpen(name: string | URL | null, fileId: { toString: (arg0: number) => any }, flags: number, pOutFlags: { set: (arg0: number) => void }) {
  ...
  xRead(fileId: any, pData: { value: { length: number; fill: (arg0: number, arg1: number) => void; subarray: (arg0: number) => any } }, iOffset: number) {
    return this.handleAsync(async () => {
      const file = this.#mapIdToFile.get(fileId)
  ...
  xWrite(fileId: any, pData: { size: number; value: Int8Array }, iOffset: number) {
    const file = this.#mapIdToFile.get(fileId)
  ...
  xTruncate(fileId: any, iSize: number) {
    const file = this.#mapIdToFile.get(fileId)
  ...
  xFileSize(fileId: any, pSize64: { set: (arg0: any) => void }) {
    const file = this.#mapIdToFile.get(fileId)
  ...
  xSectorSize(_fileId: any) {
    log('xSectorSize')
    return SECTOR_SIZE
  }

  xDeviceCharacteristics(_fileId: any) {
  ...
  xFileControl(fileId: any, op: any, _pArg: any) {
    const file = this.#mapIdToFile.get(fileId)
  ...
  xAccess(name: string, flags: any, pResOut: { set: (arg0: number) => void }) {
    return this.handleAsync(async () => {
      try {
  ...
  xDelete(name: string | URL, syncDir: any) {
    return this.handleAsync(async () => {
      const path = new URL(name, 'file://localhost/').pathname
  ...
  #maybePurge(path: string, nPages: number) {
    ...
    if (globalThis.requestIdleCallback) {
      globalThis.requestIdleCallback(() => {
        this.purge(path as IDBValidKey)
  ...
  #bound(file: any, begin: number, end = 0) {
  ...
  // appropriately.
  async #reblockIfNeeded(file: { block0: { data: any; version: any; path?: any; offset?: number }; path: IDBValidKey }) {
    const oldPageSize = file.block0.data.length
    if (oldPageSize < 18) {
      return
    } // no page size defined
    ...
    await this.#idb.run('readwrite', async ({ blocks }) => {
      ...
      for (let iOffset = 0; iOffset < fileSize; iOffset += maxPageSize) {
        ...
        if (nNewPages === 1) {
          ...
          if (newPage.offset === 0) {
            newPage = Object.assign(newPage, { fileSize })
            file.block0 = newPage
          }
          blocks.put(newPage)
        } else {
...
function openDatabase(idbDatabaseName: string): Promise<IDBDatabase> {
